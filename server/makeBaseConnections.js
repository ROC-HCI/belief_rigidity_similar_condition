//makeBaseConnection file in progress
...
//The function we call and use in callBacks.js
//takes in game object and round object (most likely not using)
function baseHelper(game, round) {
  // **Beginning of variables and arrays Declaration
  var all_nodeIds = [];
  var corresponding_likert = [];

  // -what we use to deal with algorithms
  var nodeIds_valid = [];
  var likert_valid = [];

  // this is the list we used to keep track of anyone who
  //left first stage and did not pick a scale (bad boys)
  var nodeIds_LeftMidWay = [];
  //**End of Declarations

  //**Beginning of value gathering process
  //we gather values into all the declared arrays we're going to be using
  game.players.forEach((player) => {
    //general setup - all nodeIDs, and corresponding values
    all_nodeIds.push(player.get("nodeId"));
    corresponding_likert.push(player.round.get("value"));

    //Corresponding list might have undefined values, so we do these
    // if value is not defined, meaning player left.
    // we add his nodeID into the bad boys array
    if (
      isNaN(player.round.get("value")) ||
      !Number.isInteger(Number(player.round.get("value")))
    ) {
      nodeIds_LeftMidWay.push(player.get("nodeId"));
    }
    // else, it is a valid value, we add nodeID into good boy array with corresponding scale value
    else {
      nodeIds_valid.push(player.get("nodeId"));
      likert_valid.push(player.round.get("value"));
    }
  });
  //**End of value gathering process

  //**Beginning of logic and utilization of the arrays we declared

  //variables we pass into main_work (algorithm that finds all the base_connections)
  var number_of_players = all_nodeIds.length;
  var answers = likert_valid;
  var number_of_followers = 3;
  var limit_within_bucket = 2;

  var results = main_work(
    number_of_players,
    number_of_followers,
    limit_within_bucket,
    answers,
  ...
  //how it ended up being assigned, store to database
  game.set("algo", results[2]);
  console.log("algo", game.get("algo"));
  console.log("people who left: " + nodeIds_LeftMidWay);
  //**End of logic process

  //**Begining of base connection assignment and followedPlayer attribute assignments
  for (var i = 0; i < game.players.length; i++) {
    // if we find a badboy we kept track of, we do random assignment for him
    if (nodeIds_LeftMidWay.includes(game.players[i].get("nodeId"))) {
      var player_nodeID = game.players[i].get("nodeId");
      // game.players[i].set("base_connections", results[0][i]);
      game.players[i].set("followed_by", results[1][i]);
      console.log("this is the bad boy: " + player_nodeID);

      var number_of_connections = 3;
      // only pass in nodeIds_valid so the people who did not pick a scale will only
      // get random connections of players who did pick a scale.
      // nodeIDs_valid excludes this player himself and others who did not pick a scale
      var randomBase = randomAssignment(nodeIds_valid, number_of_connections);

      game.players[i].set("base_connections", randomBase);
      // console.log(
      //   "this is randomly assigned since they did not pick a scale"
      // );
      // console.log(game.players[i].get("base_connections"));
      // console.log(game.players[i].get("followed_by"));
    }
    //if a good boy, we assign the connection we found for him through our algorithm
    else {
      game.players[i].set("base_connections", results[0][i]);
      game.players[i].set("followed_by", results[1][i]);
      ...
      // console.log(
      //   "player nodeID: " +
      //     game.players[i].get("nodeId") +
      //     ", scale value: " +
      //     game.players[i].round.get("value") +
      //     ", name: " +
      //     game.players[i].id
      // );
      // console.log("following(base_connections): " + game.players[i].get("base_connections"));
      // console.log("followed by: " + game.players[i].get("followed_by"));
    }

    // set followedPlayers as well,
    // followedPlayers used for rendering button text along with total rec_list later in reactionResponse
    var base_connection_for_followed = game.players[i].get("base_connections");
    game.players[i].round.set("followedPlayers", base_connection_for_followed);
  }
  //**End of base connection assignment
}
